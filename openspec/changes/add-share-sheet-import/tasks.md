...
- [ ] Extract the JSON-LD import pipeline (network fetch, parser, mapping) into a module that both the app target and extension can import without duplicating code.
- [ ] Implement an extension view model that resolves shared URL attachments, runs the importer with async/await, and produces a mutable `RecipeDraft`.
