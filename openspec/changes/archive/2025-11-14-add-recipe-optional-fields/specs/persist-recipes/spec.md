...
- **AND** the migration MUST ensure the following tables exist:
  - `recipes` with columns for id, title, summary, prepTimeMinutes, cookTimeMinutes, servings, createdAt, updatedAt, notes (optional), nutrition (optional), and website (optional)
  ...
  - `recipe_photos` with columns for id, recipeId, position, and photoData, with a foreign key to `recipes.id` and uniqueness on `(recipeId, position)`
...
- **AND** the model MUST include optional `notes`, `nutrition`, and `website` fields

#### Scenario: Storage exposes photo collection
- **GIVEN** a recipe has associated photos
- **WHEN** views fetch the recipe details
- **THEN** they MUST receive an ordered collection of `RecipePhoto` values with position metadata
- **AND** photos MUST be sorted by position in ascending order
- **AND** the photo data MUST be accessible for display purposes
