...
#### Scenario: Persist mapped recipe
...
- **THEN** it MUST create a new `Recipe` with a generated UUID, title from `name`, summary from `description`, newline-joined ingredients from `recipeIngredient`, and newline-separated instructions derived from `recipeInstructions`
- **AND** it MUST convert ISO8601 duration strings (`prepTime`, `cookTime`) into minute counts when present
- **AND** it MUST store the recipe via the shared SQLiteData writer, returning the saved `Recipe` so callers can present it
