...
extension RecipeImportManager: DependencyKey {
  static let liveValue: RecipeImportManager = RecipeImportManager { url in
    ...
    let html = try await fetchHTML(from: url, session: urlSession)
    let recipeJSON = try extractRecipeJSON(from: html)
    let importedRecipe = try await ImportedRecipe(json: recipeJSON)
...
extension RecipeImportManager {
  fileprivate nonisolated static func fetchHTML(from url: URL, session: URLSession) async throws
    -> String
  {
  ...
  fileprivate nonisolated static func extractRecipeJSON(from html: String) throws -> [String: Any] {
  ...
  fileprivate nonisolated static func collectRecipeCandidates(from object: Any) -> [[String: Any]] {
  ...
  fileprivate nonisolated static func typeContainsRecipe(_ value: Any?) -> Bool {
    switch value {
...
enum RecipeImportError: Error, LocalizedError, Equatable {
...
// MARK: - Debug Harness
  ...
  enum RecipeImportHarness {
    ...
    static func importSampleRecipe(using database: any DatabaseWriter) throws -> Recipe {
      ...
      let recipeJSON = try RecipeImportManager.extractRecipeJSON(from: sampleHTML)
